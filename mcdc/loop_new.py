...
@njit
def loop_fixed_source(data_tally, mcdc_arr, data_new, mcdc_new):
    ...
    for idx_batch in range(mcdc["setting"]["N_batch"]):
        ...
        for idx_census in range(mcdc["setting"]["N_census"]):
            ...
            loop_source(seed_source, data_tally, mcdc, mcdc_new)
...
@njit
def exhaust_active_bank(data_tally, prog, mcdc_new):
    mcdc = adapt.mcdc_global(prog)
    ...
    while kernel.get_bank_size(mcdc["bank_active"]) > 0:
        ...
        # Particle loop
        loop_particle(P_arr, data_tally, mcdc, mcdc_new)
...
@njit
def loop_source(seed, data_tally, mcdc, mcdc_new):
...
@njit
def loop_particle(P_arr, data_tally, prog, mcdc_new):
    P = P_arr[0]
    mcdc = adapt.mcdc_global(prog)

    while P["alive"]:
        step_particle(P_arr, data_tally, prog, mcdc_new)


@njit
def step_particle(P_arr, data_tally, prog, mcdc_new):
    P = P_arr[0]
    mcdc = adapt.mcdc_global(prog)

    # Determine and move to event
    kernel.move_to_event(P_arr, data_tally, mcdc, mcdc_new)
