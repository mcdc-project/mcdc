...
def array_return_typing(fn,elem_type):
    ...
    lns = {}
    arg_list = ",".join([f"param_{i}" for i in range(param_count)])
...
#@nb.extending.type_callable(attribute_all)
def type_attribute_all(context):
    from numba.np.numpy_support import as_dtype

    context_guard(context)   
    
    def typer(thing, data):
        thing_dtype = as_dtype(thing)
        if not thing_dtype == thing_type:
            raise nb.core.errors.TypingError(f"First argument is type {thing_dtype}, but must be type {thing_type}.")
        if not isinstance(data,nb.types.Array):
            raise nb.core.errors.TypingError(f"Second argument is type {data_dtype}, but must be an Array of type {data_type}.")
        data_dtype  = as_dtype(data.dtype)
        if not data_dtype == data_type:
            raise nb.core.errors.TypingError(f"Second argument has element dtype {data_dtype}, but must be type {data_type}.")
        elif not data.ndim == 1:
            raise nb.core.errors.TypingError(f"Second argument should be one-dimensional.")
        elif not data.layout == "C":
            raise nb.core.errors.TypingError(f"Second argument should have C layout.")
            
        
        retty = types.Array(dtype=data.dtype,ndim=1,layout="C")
        sig = retty(thing,data)
        return sig 

    return typer


#@nb.extending.lower_builtin(attribute_all,nb.from_dtype(thing_type),nb.types.Array(dtype=nb.from_dtype(data_type),ndim=1,layout="C"))
def builtin_attribute_all(context, builder, sig, args):
    ...
    try:
        ...
        raise nb.core.errors.UnsupportedError(f"ROCM ain't available.")
        ROCM_AVAILABLE = False
    ...
    res = context.compile_internal(builder,attribute_all_ptr.py_func,ptr_sig,args)
    ptr_res  = builder.extract_value(res,0)
    size_res = builder.extract_value(res,1)
    shape = [size_res]
    dtype = data_type.dtype
    ...
    aryty = types.Array(dtype=dtype, ndim=1, layout="C")
    ary = context.make_array(aryty)(context, builder)
...
thing_a_type = np.dtype([
    ("attribute_a_offset",np.intp),
    ("attribute_a_size", np.intp),
])

thing_b_type = np.dtype([
    ("attribute_b_offset",np.intp),
    ("attribute_b_size", np.intp),
])


# We're cooking with gas now! >B)

@array_return(nb.float64)
def attribute_a_all(thing, data):
    start = thing["attribute_a_offset"]
    size  = thing["attribute_a_size"] 
    end   = start + size
    return array_result(data[start:end])


@array_return(nb.float64)
def attribute_b_all(thing, data):
    start = thing["attribute_b_offset"]
    size  = thing["attribute_b_size"] 
    end   = start + size
    return array_result(data[start:end])
...
@njit()
def check(thing_a,thing_b,data):
    a = attribute_a_all(thing_a,data)    
    b = attribute_b_all(thing_b,data)    
    print(a)
    print(b)


@nb.hip.jit()
def hip_check(thing_a,thing_b,data):
    a = attribute_a_all(thing_a,data)    
    b = attribute_b_all(thing_b,data)    
...
data = np.zeros((10,),dtype=data_type)
for i in range(10):
    data[i] = i

thing_a_arr = np.zeros((1,),thing_a_type)
thing_a = thing_a_arr[0]
thing_a["attribute_a_offset"] = 4
thing_a["attribute_a_size"]   = 3
thing_b_arr = np.zeros((1,),thing_b_type)
thing_b = thing_b_arr[0]
thing_b["attribute_b_offset"] = 8
thing_b["attribute_b_size"]   = 1

check(thing_a,thing_b,data)
hip_check[1,1](thing_a,thing_b,data)
