...
#====
# Add cgmfwrap.so module. Set CGMF environment variable to the location of cgmfwrap.so
#====
import sys
import os

sys.path.append('/nfs/stak/users/agnellj/project/wrapper/cgmfwrap/build')
import cgmfwrap
    ...
    for n in range(N):
        ...
        if not angle_type == ANGLE_ENERGY_CORRELATED:
            E_new = sample_distribution(
                E, spectrum_base, particle_container_new, mcdc, data
            )
           # E_new = sample_distribution(
...
# ==============================================================================
 ...
 # ==============================================================================
    ...
    #N_delayed = 1 if xi < nu_d else 0   # ask about this !!
    N_delayed = np.random.poisson(nu_d)
    N_groups = nuclide["N_neutron_fission_delayed_precursor"] # number of precursor groups
        
    
    # compute total delayed neutron fraction
    beta_total = 0.0
    ...
    for n in range(N_delayed):
        ...
        for j in range(N_groups):
            beta_j = mcdc_get.nuclide.neutron_fission_delayed_fractions(
                j, nuclide, data
            )
    
            P_j += beta_j / beta_total
