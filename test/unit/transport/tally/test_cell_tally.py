...
def test_cell_tally_sign_for_incoming_and_outgoing(slab_plane_x, crossing_particle):
...
def test_cell_tally_records_in_and_out_separately(slab_plane_x, crossing_particle):
...
def test_cell_tally_counts_outgoing_to_vacuum(slab_plane_x, crossing_particle):
    cell_tally_obj = mcdc.Tally(
        cell=slab_plane_x["c_right"],
        scores=["net-current", "current-in", "current-out"],
    )
    ...
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 0), -2.0)
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 1), 0.0)
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 2), 2.0)


def test_cell_tally_ignores_reflective_crossing(material_mg, crossing_particle):
    ...
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 0), 0.0)
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 1), 0.0)
    assert np.isclose(_bin_value_score(cell_tally, mcdc_struct, data, 2), 0.0)


def test_cell_tally_scores_curved_boundary(material_mg, crossing_particle):
